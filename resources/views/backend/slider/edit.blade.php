...
<div class="panel shadow-sm">
    <div class="panel-body">
		...
		@if($data->files->where('alias', 'gambar_slider')->where('fillable_id', $data->id)->count())
			<div class='form-group'>
				<table class="table table-{!! $data->id !!}">
					@foreach($data->files->where('alias', 'gambar_slider')->where('fillable_id', $data->id) as $gambar)
						@if($gambar->exists())
							<tr>
								<td>
									File :
