...
<div class="panel shadow-sm">
    <div class="panel-body">
		...
		<div class='form-group'>
			{!! html()->label()->class('control-label')->for('tugas')->text('Tugas') !!}
			{!! html()->text('tugas',$data->tugas)->placeholder('Type Tugas here')->class('form-control')->id('tugas') !!}
		</div>
		<div class="form-group">
		...
		@if($data->files->where('alias', 'gambar_struktur')->where('fillable_id', $data->id)->count())
			<div class='form-group'>
				<table class="table table-{!! $data->id !!}">
					@foreach($data->files->where('alias', 'gambar_struktur')->where('fillable_id', $data->id) as $gambar)
						@if($gambar->exists())
							<tr>
								<td>
									File :
