...
<div class="panel shadow-sm">
    <div class="panel-body">
		...
		<div class='form-group'>
			...
			{!! html()->text('penyelenggara',$data->penyelenggara)->placeholder('Type Penyelenggara here')->class('form-control')->id('penyelenggara') !!}
		</div>
		<div class='form-group'>
		...
		@if($data->files->where('alias', 'gambar_penghargaan')->where('fillable_id', $data->id)->count())
			<div class='form-group'>
				<table class="table table-{!! $data->id !!}">
					@foreach($data->files->where('alias', 'gambar_penghargaan')->where('fillable_id', $data->id) as $gambar)
						@if($gambar->exists())
							<tr>
								<td>
									File :
