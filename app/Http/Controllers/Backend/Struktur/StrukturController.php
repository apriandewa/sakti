...
namespace App\Http\Controllers\Backend\Struktur;
...
class StrukturController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($struktur = $this->model::create($data)) {
            
            // Handle file upload if 'gambar' is present in the request
            if ($request->hasFile('gambar')) {
                    $struktur->file()->create([
                        'data' => [
                            'name' => $request->file('gambar')->getClientOriginalName(),
                            'disk' => config('filesystems.default'),
                            'target' => Storage::disk(config('filesystems.default'))->putFile('struktur', $request->file('gambar')),
                        ],
                        'alias' => 'gambar_struktur', // Menambahkan alias untuk field gambar
                    ]);
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle file upload if 'gambar' is present in the request
            if($data->update($request->all())){
                if ($request->hasFile('gambar')) {
                    // Hapus semua file lama dengan alias 'gambar_struktur'
                    foreach ($data->files()->where('alias', 'gambar_struktur')->get() as $file) {
