...
namespace App\Http\Controllers\Backend\Tautan;
...
class TautanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($tautan = $this->model::create($data)) {
            ...
            if ($request->hasFile('gambar')) {
                    $tautan->file()->create([
                        'data' => [
                            ...
                            'target' => Storage::disk(config('filesystems.default'))->putFile('tautan', $request->file('gambar')),
                        ],
                        'alias' => 'gambar_tautan', // Menambahkan alias untuk field gambar
                    ]);
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle file upload if 'gambar' is present in the request
            if($data->update($request->all())){
                if ($request->hasFile('gambar')) {
                    // Hapus semua file lama dengan alias 'gambar_tautan'
                    foreach ($data->files()->where('alias', 'gambar_tautan')->get() as $file) {
