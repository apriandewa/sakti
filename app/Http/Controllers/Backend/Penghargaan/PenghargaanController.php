...
namespace App\Http\Controllers\Backend\Penghargaan;
...
class PenghargaanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($penghargaan = $this->model::create($data)) {
            
            // Handle file upload if 'gambar' is present in the request
            if ($request->hasFile('gambar')) {
                    $penghargaan->file()->create([
                        'data' => [
                            'name' => $request->file('gambar')->getClientOriginalName(),
                            'disk' => config('filesystems.default'),
                            'target' => Storage::disk(config('filesystems.default'))->putFile('penghargaan', $request->file('gambar')),
                        ],
                        'alias' => 'gambar_penghargaan', // Menambahkan alias untuk field gambar
                    ]);
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle file upload if 'gambar' is present in the request
            if($data->update($request->all())){
                if ($request->hasFile('gambar')) {
                    // Hapus semua file lama dengan alias 'gambar_penghargaan'
                    foreach ($data->files()->where('alias', 'gambar_penghargaan')->get() as $file) {
