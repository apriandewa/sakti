...
class PageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($page = $this->model::create($data)) {
            ...
            if ($request->hasFile('gambar')) {
                    $page->file()->create([
                        'data' => [
                            ...
                            'target' => Storage::disk(config('filesystems.default'))->putFile('page', $request->file('gambar')),
                        ],
                        'alias' => 'gambar_page', // Menambahkan alias untuk field gambar
                    ]);
                }
       ...
       $data=$this->model::find($id);
        // Handle file upload if 'gambar' is present in the request
            if($data->update($request->all())){
                if ($request->hasFile('gambar')) {
                    // Hapus semua file lama dengan alias 'gambar_page'
                    foreach ($data->files()->where('alias', 'gambar_page')->get() as $file) {
