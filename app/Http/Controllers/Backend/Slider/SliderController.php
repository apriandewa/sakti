...
class SliderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($slider = $this->model::create($data)) {
            ...
            if ($request->hasFile('gambar')) {
                    $slider->file()->create([
                        'data' => [
                            ...
                            'target' => Storage::disk(config('filesystems.default'))->putFile('slider', $request->file('gambar')),
                        ],
                        'alias' => 'gambar_slider', // Menambahkan alias untuk field gambar
                    ]);
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle file upload if 'gambar' is present in the request
            if($data->update($request->all())){
                if ($request->hasFile('gambar')) {
                    // Hapus semua file lama dengan alias 'gambar_slider'
                    foreach ($data->files()->where('alias', 'gambar_slider')->get() as $file) {
